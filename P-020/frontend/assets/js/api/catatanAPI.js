const BASE_URL = "http://localhost:7000/api/v1/catatan";
...
export async function restoreCatatan(id) {
  try {
    const res = await fetch(`${BASE_URL}/restore/${id}`, { method: "PATCH" });
    return await res.json();
  } catch (err) {
...
export async function forceDeleteCatatan(id) {
  try {
    const res = await fetch(`${BASE_URL}/force/${id}`, { method: "DELETE" });
    return await res.json();
  } catch (err) {
